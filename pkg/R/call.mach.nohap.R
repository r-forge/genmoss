...
if (step2.subjects > 0) {
	...
	while((subjects.done+step2.subjects) < nrow(P)) {
		...
		# If the step2 crashed, re-do it
		if(!file.exists(paste(dir.debug, "/", prefix2, ".mlgeno", sep="")) && step2.subjects > 2) {
print("FAILED.... redoing...")
			step2.subjects <- floor(step2.subjects * 0.5)			# <---------------------------------------
		} else {
...
print(p.name)
	...
	tocombine <- paste(tocombine, "> ", touse)
	system(tocombine)
