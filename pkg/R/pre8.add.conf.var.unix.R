pre8.add.conf.var.unix <- function(file.name, dir.file, file.fam, dir.fam=dir.file, file.conf, dir.conf=dir.file, file.out, fam.out=file.fam, dir.out) {
...
if(missing(fam.out)) stop("Name of the output family file must be provided")
...
new.name <- "tmpN.txt"
write.table(newN, file=paste(dir.out, new.name, sep="/"), col.names=FALSE, row.names=FALSE, quote=FALSE, sep="\t")
...
#binN <- read.table(new.name.bin$train, sep=" ", stringsAsFactors=FALSE)

command.paste <- paste("cut -f 1-", (ped.dims$ncols-1), " ", file.ped.tmp, " | paste - ", new.name.bin$train, " > ", dir.out, "/", file.out, sep="")
print(command.paste)
...
#file.remove(paste(dir.out, new.name, sep="/"))
#file.remove(new.name.bin$train)
file.remove(new.name.bin$test)
