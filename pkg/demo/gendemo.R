...
?"Create the directory structure: will return names of folders: d0 to d10"
dirs <- pre0.dir.create(".", "demo.genos", "dir")

?"Find directory where original files are and copy them into d0 folder"
...
?"Download, unzip, and move into d4 folder the reference files for MaCH1 (optional step, useful for improving quality of imputation of missing values). Here we demonstrate this for chromosome number 21."
system("wget http://hapmap.ncbi.nlm.nih.gov/downloads/phasing/2007-08_rel22/phased/genotypes_chr21_CEU_r22_nr.b36_fwd.phase.gz")
system("wget http://hapmap.ncbi.nlm.nih.gov/downloads/phasing/2007-08_rel22/phased/genotypes_chr21_CEU_r22_nr.b36_fwd_legend.txt.gz")
system("gzip -d genotypes_chr21_CEU_r22_nr.b36_fwd.phase.gz")
system("gzip -d genotypes_chr21_CEU_r22_nr.b36_fwd_legend.txt.gz")
system(paste("mv genotypes_chr21_CEU_r22_nr.b36_fwd.phase ", dirs$d4, sep=""))
system(paste("mv genotypes_chr21_CEU_r22_nr.b36_fwd_legend.txt ", dirs$d4, sep=""))

?"Run MaCH1 WITH Hapmap (for chromosome 21) to accurately impute missing values in the dataset, saving the result to d5 folder. 2 runs: for CASE and then for CONTROL."
prefix.ped.case <- "genotypes_CASE_chr"         # will be used as new prefix
prefix.ped.control <- "genotypes_CONTROL_chr"   # will be used as new prefix

pre3.call.mach.batch(dir.file=dirs$d3, dir.ref=dirs$d4, dir.out=dirs$d5, prefix.dat=prefix.dat, prefix.ped=prefix.ped, prefix.phase="genotypes_chr", prefix.out=prefix.ped.case, key.ped="CASE", num.iters=1, num.subjects=20, step2.subjects=25, empty="N/N", chrom.num=21)

pre3.call.mach.batch(dir.file=dirs$d3, dir.ref=dirs$d4, dir.out=dirs$d5, prefix.dat=prefix.dat, prefix.ped=prefix.ped, prefix.phase="genotypes_chr", prefix.out=prefix.ped.control, key.ped="CONTROL", num.iters=1, num.subjects=20, step2.subjects=25, empty="N/N", chrom.num=21)

?"Run MaCH1 without hapmap for all other chromosomes, saving result to d5 folder. Since this dataset is small, we do not need to split it for processing, so set all numbers of subjects to 0 (meaning use everyone, 25 subjects). Also this is the only batch function that does not process all chromosomes (since it's best to do this in parallel)."
i <- 1
while(i<=22) {
	if(i != 21) {  # processed 21st already
